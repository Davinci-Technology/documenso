...
| `payload.source`                             | string    | The source of the document (e.g., DOCUMENT).           |
| `payload.documentMeta`                       | json?     | Metadata for the document.                            |
| `payload.emailId`                            | string?   | Custom email ID if set.                               |
| `payload.envelopeId`                         | number?   | The id of the envelope if document belongs to one.    |
| `payload.recipients`                         | array     | List of recipients and their signature status.        |
| `payload.recipients.id`                      | number    | Recipient ID.                                         |
| `payload.recipients.email`                   | string    | Recipient email.                                      |
| `payload.recipients.name`                    | string    | Recipient name.                                       |
| `payload.recipients.role`                    | string    | Recipient role (e.g., SIGNER, VIEWER).                |
| `payload.recipients.readStatus`              | string    | Recipient read status (e.g., NOT_OPENED).             |
| `payload.recipients.signingStatus`           | string    | Recipient signing status (e.g., NOT_SIGNED).          |
| `payload.recipients.signingOrder`            | number?   | Order in which the recipient should sign.             |
| `payload.recipients.fields`                  | array     | List of fields assigned to the recipient.             |
| `payload.recipients.fields.id`               | number    | Field ID.                                             |
| `payload.recipients.fields.type`             | string    | Field type (e.g., SIGNATURE, DATE, TEXT).             |
| `payload.recipients.fields.value`            | string?   | Value of the field if filled.                         |
| `payload.recipients.fields.pageNumber`       | number    | Page number where the field is located.               |
| `payload.recipients.fields.required`         | boolean   | Whether the field is required.                        |
| `payload.recipients.fields.customText`       | string?   | Custom text for the field if applicable.              |
| `payload.recipients.fields.inserted`         | boolean   | Whether the field has been filled/inserted.           |
| `payload.recipients.fields.fieldMeta`        | json?     | Metadata for the specific field.                      |
| `payload.recipients.fields.validatorOptions` | json?     | Validation rules for the field.                       |
