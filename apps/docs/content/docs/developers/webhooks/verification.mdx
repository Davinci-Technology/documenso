...
When you configure a webhook with a secret, Documenso includes that secret in every webhook request via the `X-Davinci-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-Davinci-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-Davinci-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-Davinci-Secret` header from the incoming request
  </Step>
  <Step>
...
def handle_webhook():
    received_secret = request.headers.get('X-Davinci-Secret', '')
