...
- A Davinci Sign account on any plan (Free, Individual, Team, or Enterprise)

<Callout type="info">
  Free accounts include API access with a limit of 5 documents per month. [Upgrade to a paid
  plan](https://davincisolutions.ai/pricing) for higher limits.
</Callout>
...
Log in to Davinci Sign and open the team that the integration should access. API tokens are scoped to a
team.
...
```bash
curl https://app.davincisolutions.ai/api/v2/envelope \
...
Davinci Sign provides official SDKs that handle authentication for you:

```typescript
import { Davinci Sign } from '@documenso/sdk-typescript';

const client = new Davinci Sign({
...
<Callout type="info">
  ...
  OpenAPI reference](https://app.davincisolutions.ai/api/v1/openapi).
</Callout>
...
DAVINCI_API_KEY=api_xxxxxxxxxxxxxxxx
```
