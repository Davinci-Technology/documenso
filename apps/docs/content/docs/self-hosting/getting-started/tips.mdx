...
The default certificate path inside the container is `/opt/davinci-sign/cert.p12`. You can change this with `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`.
...
```bash
docker run -it -e HOST=:: davinci/davinci-sign:latest npm run start
```
