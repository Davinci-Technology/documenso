...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`      | Path to signing certificate inside container                   | `/opt/davinci-sign/cert.p12` |
...
- **Volume mount** — mount a `.p12` file from the host into the container at `/opt/davinci-sign/cert.p12` (shown above). This is the simplest approach for small to moderate deployments.
