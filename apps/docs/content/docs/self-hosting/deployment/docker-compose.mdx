...
curl -O https://raw.githubusercontent.com/davinci/davinci-sign/release/docker/production/compose.yml
```
...
services:
  ...
  documenso:
    image: davinci/davinci-sign:latest
    depends_on:
      database:
    ...
    ports:
      - ${PORT:-3000}:${PORT:-3000}
    volumes:
      - /opt/davinci-sign/cert.p12:/opt/davinci-sign/cert.p12:ro
...
sudo cp /path/to/your/cert.p12 /opt/davinci-sign/cert.p12
sudo chown 1001:1001 /opt/davinci-sign/cert.p12
sudo chmod 400 /opt/davinci-sign/cert.p12
```
...
```
NAME                             STATUS              PORTS
davinci-sign-production-database-1  running (healthy)   5432/tcp
davinci-sign-production-documenso-1 running             0.0.0.0:3000->3000/tcp
```
