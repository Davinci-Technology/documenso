...
|------|--------|--------------|---------|
| **Build** (B1) | `build_gate.py` (Jenkins `Post-Merge Gates` stage) | `build_gate_failures.txt` | Any non-compiling merge. First runs a **full-repo `npm ci`** (with the engines-required npm) to catch lockfile/dependency drift in ANY workspace — the pruned remix build alone would miss e.g. `apps/docs` drift. Then builds the final tree (`turbo run build`, incl. `tsc --noEmit`). **Self-heals** compile failures: feeds per-file errors to the repair LLM, rebuilds, up to 2 rounds; escalates to a draft PR when a round changes nothing or errors span multiple files. Lockfile drift is not auto-fixed (needs a regen) — it blocks straight to review. |
