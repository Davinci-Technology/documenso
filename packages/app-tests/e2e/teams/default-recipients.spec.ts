...
test.describe('Default Recipients', () => {
  test('[DEFAULT_RECIPIENTS]: default recipients are added to documents created via UI', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').first().fill('regular-signer@davincisolutions.ai');
    await page
    ...
    await expect(async () => {
      ...
      const regularSigner = envelope.recipients.find((r) => r.email === 'regular-signer@davincisolutions.ai');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created via API have default recipients', async ({ request }) => {
    ...
    const payload: TCreateEnvelopePayload = {
      ...
      recipients: [
        {
          email: 'api-recipient@davincisolutions.ai',
          name: 'API Recipient',
    ...
    const apiRecipient = envelope.recipients.find((r) => r.email === 'api-recipient@davincisolutions.ai');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created from template have default recipients', async ({ page }) => {
    ...
    await page.getByPlaceholder('Email').fill('template-recipient@davincisolutions.ai');
    ...
    const templateRecipient = document.recipients.find((r) => r.email === 'template-recipient@davincisolutions.ai');
